...
public class DefaultClientConnectionOperator implements ClientConnectionOperator {
    ...
    */
    public void openConnection(OperatedClientConnection operatedClientConnection, HttpHost httpHost, InetAddress inetAddress, HttpContext httpContext, HttpParams httpParams) throws IOException {
        ...
        DefaultClientConnectionOperator defaultClientConnectionOperator = this;
        ...
        SchemeRegistry schemeRegistry2 = defaultClientConnectionOperator.getSchemeRegistry(httpContext);
        Scheme scheme = schemeRegistry2.getScheme(httpHost.getSchemeName());
        SchemeSocketFactory schemeSocketFactory = scheme.getSchemeSocketFactory();
        InetAddress[] resolveHostname = defaultClientConnectionOperator.resolveHostname(httpHost.getHostName());
        int resolvePort = scheme.resolvePort(httpHost.getPort());
        int i2 = 0;
        while (i2 < resolveHostname.length) {
            InetAddress inetAddress3 = resolveHostname[i2];
            boolean z = i2 == resolveHostname.length - i;
            Socket createSocket = schemeSocketFactory.createSocket(httpParams);
            operatedClientConnection.opening(createSocket, httpHost);
            HttpInetSocketAddress httpInetSocketAddress2 = new HttpInetSocketAddress(httpHost, inetAddress3, resolvePort);
            InetSocketAddress inetSocketAddress = inetAddress2 != null ? new InetSocketAddress(inetAddress2, 0) : null;
            if (defaultClientConnectionOperator.log.isDebugEnabled()) {
                Log log = defaultClientConnectionOperator.log;
                StringBuilder sb = new StringBuilder();
                schemeRegistry = schemeRegistry2;
                sb.append("Connecting to ");
                httpInetSocketAddress = httpInetSocketAddress2;
                sb.append(httpInetSocketAddress);
            ...
            try {
                ...
                defaultClientConnectionOperator.prepareSocket(createSocket, httpContext, httpParams);
                ...
                if (!defaultClientConnectionOperator.log.isDebugEnabled()) {
                    defaultClientConnectionOperator.log.debug("Connect to " + httpInetSocketAddress + " timed out. Connection will be retried using another IP address");
                }
                i2++;
                defaultClientConnectionOperator = this;
