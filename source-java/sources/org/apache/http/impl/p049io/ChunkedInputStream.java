...
public class ChunkedInputStream extends InputStream {
    ...
    /* renamed from: in */
    private final SessionInputBuffer f5036in;
    private int pos;
    private int state;
    ...
    public ChunkedInputStream(SessionInputBuffer sessionInputBuffer, MessageConstraints messageConstraints) {
        ...
        this.f5036in = (SessionInputBuffer) Args.notNull(sessionInputBuffer, "Session input buffer");
        this.pos = 0;
    ...
    private int getChunkSize() throws IOException {
        ...
        if (i != 1) {
            ...
            this.buffer.clear();
            if (this.f5036in.readLine(this.buffer) == -1) {
    ...
    private void parseTrailerHeaders() throws IOException {
        try {
            this.footers = AbstractMessageParser.parseHeaders(this.f5036in, this.constraints.getMaxHeaderCount(), this.constraints.getMaxLineLength(), null);
    ...
    public int available() throws IOException {
        SessionInputBuffer sessionInputBuffer = this.f5036in;
    ...
    public int read(byte[] bArr, int i, int i2) throws IOException {
        ...
        int read = this.f5036in.read(bArr, i, Math.min(i2, this.chunkSize - this.pos));
        if (read != -1) {
            this.pos += read;
            if (this.pos >= this.chunkSize) {
                this.state = 3;
